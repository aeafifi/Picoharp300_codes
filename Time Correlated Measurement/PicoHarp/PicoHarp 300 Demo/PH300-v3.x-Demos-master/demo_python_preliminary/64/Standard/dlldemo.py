...
# From phdefin.h
LIB_VERSION = "3.0"
HISTCHAN = 65536 #number of histrogram channels DO NOT CHANGE
MAXDEVNUM = 8 #maximum device number
MODE_HIST = 0
FLAG_OVERFLOW = 0x0040

# Measurement parameters, these are hardcoded since this is just a demo
binning = 4 # you can change this: from 0-7 0=4ps, 7=512ps
binWidth = 4*(2**binning)
offset = 0
tacq = 90000 # Measurement time in millisec, you can change this
...
#Writes text file for data
outputfile = open("dlldemo.txt", "w+")

#Adds given settings into output file
...
while cmd != "q":
    ...
    tryfunc(phlib.PH_GetCountRate(ct.c_int(dev[0]), ct.c_int(0), byref(countRate0)),\
            "GetCountRate") #channel 0
    tryfunc(phlib.PH_GetCountRate(ct.c_int(dev[0]), ct.c_int(1), byref(countRate1)),\
            "GetCountRate") #channel 1
    
    print("Countrate0=%d/s Countrate1=%d/s" % (countRate0.value, countRate1.value))
    
    times, tacq = init_scan() #taqu in s rather than ms
    print("\nScan Initialised...")
    
    print("\nMeasuring for %d seconds..." % tacq)
    
    
    tryfunc(phlib.PH_StartMeas(ct.c_int(dev[0]), ct.c_int(tacq*1000)), "StartMeas")
    begin_scan(times)
    waitloop = 0
...
outputfile.close()
displayCounts('dlldemo', tacq, binWidth)
#closeDevices()
